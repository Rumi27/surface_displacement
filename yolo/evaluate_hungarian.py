...
yolo_dir = Path("results_hungarian")  # Hungarian matcher results
...
for case in cases:
    ...
    # Create marker_label for Hungarian-based results
    if "marker_id_before" in yolo.columns:
        yolo["marker_label"] = yolo["marker_id_before"].astype(str).str.strip().str.upper()
    elif "marker_label" in yolo.columns:
        yolo["marker_label"] = yolo["marker_label"].astype(str).str.strip().str.upper()
    else:
        raise ValueError("No marker identifier found in YOLO file.")

    df = pd.merge(gt, yolo, on="marker_label", how="inner")

    if df.empty:
        print(f"[WARN] no overlap for {case}")
        continue
